/*
	...
	We use the testing algorithm from Problem 105, and try different sets.
	From Problem 105 we know that the sum must be leq 291 
*/
...
var distinctSubsetPairsOfDifferentLength [][][2][]int
...
func main() {
	...
	for i_1 := 1; i_1 <= LIMIT; i_1++ {
		for i_2 := i_1+1; i_2 <= LIMIT; i_2++ {
			for i_3 := i_2+1; i_3 <= LIMIT; i_3++ {
				for i_4 := i_3+1; i_4 <= LIMIT; i_4++ {
					for i_5 := i_4+1; i_5 <= LIMIT; i_5++ {
						for i_6 := i_5+1; i_6 <= LIMIT; i_6++ {
							for i_7 := i_6+1; i_7 <= LIMIT; i_7++ {

								set := []int{i_1, i_2, i_3, i_4, i_5, i_6, i_7}					
								
								if (set[0] + set[1]) > set[6] {
									if setSum(set) < smallestSum {
										if testSet(set) {
											smallestSum = setSum(set)
											smallestSet = set
										}
									}			
								}
	...
	fmt.Println(smallestSet, smallestSum)
}

func testSet(set []int) bool {
	n := len(set)

	// we first assume that property B is satisfied, and check if then property A is satisfied. For this, we only need to check
	// disjoint subsets of the same size
	distinctNSubsetKPairsOfSameLength := distinctSubsetPairsOfSameLength[n-7]

	for _, distinctKPairsOfSameLength := range distinctNSubsetKPairsOfSameLength {
		for _, distinctPairOfSameLength := range distinctKPairsOfSameLength {
			if subsetSum(set, distinctPairOfSameLength[0]) == subsetSum(set, distinctPairOfSameLength[1]) {
	...
	// now check if property B is satisfied
	distinctNSubsetPairsOfDifferentLength := distinctSubsetPairsOfDifferentLength[n-7]

	for _, distinctPairOfDifferentLength := range distinctNSubsetPairsOfDifferentLength {
		if subsetSum(set, distinctPairOfDifferentLength[0]) >= subsetSum(set, distinctPairOfDifferentLength[1]) {
			return false
		}
	}
...
func makeNeededSubsets() {
	...
	// Step 3: From n = 7 to 12, make all disjoint pairs of different length
	for n := 7; n <= 12; n++ {
		subsetsForN := subsets[n-7]
		...
		for k := 2; n-k > k; k++ {
			...
			for l := k+1; k+l <= n; l++ {
				nChooseLSets := subsetsForN[l-2]

				for _, kSet := range nChooseKSets {
					for _, lSet := range nChooseLSets {
						if setsAreDisjoint(kSet, lSet) {
							disjointPairsForN = append(disjointPairsForN, [2][]int{kSet, lSet})
						}
					}
				}
			} 
		...
		distinctSubsetPairsOfDifferentLength = append(distinctSubsetPairsOfDifferentLength, disjointPairsForN)
	}
}
...
// given a parent set, sums up the elements that are represented by the index subset.
// i.e. parentSet = 2,3,4 and indexSubset = 2,3 returns 3+4 = 7
func subsetSum(parentSet []int, indexSubset []int) int {
	sum := 0
	for _, i := range indexSubset {
		sum += parentSet[i - 1]
	}
