/*
	...
	Our approach is to find circles and always remove the biggest edge. 
	We start by removing triangle circles and then the rest, until there are no more.
*/
...
func graphIsConnected() bool {
	...
	for len(nodesToCheck) >= 1 {
		...
		for B := 0; B<N; B++ {
			if graph[A][B] > -1 {
				canReach[B] = 1
				if !crabMath.IntSliceContains(checked, B) {
					nodesToCheck = append(nodesToCheck, B)
				}
			}
		}
	}

	return !sliceContainsZero(canReach)
}

func sliceContainsZero(s []int) bool {
	for _, v := range s {
		if v == 0 {
			return true
		}
...
func removeDeepSearchCircles() {
	// the graph is connected, so we start at the first node and walk through the graph. as soon as we find a circle, we remove it. if there is a crossroad, we split

	brokeCircle := true

	slitherCount := 0
	for brokeCircle == true {
		brokeCircle = false
		slither([]int{0}, &brokeCircle)
		slitherCount++
	}

	fmt.Println("slither count", slitherCount)
	
}

func slither(snake []int, closedCircle *bool) {
	// A is the last node in the snake
	A := snake[len(snake) - 1]


	// iterate over all edges connecting A
	for B := 0; B < N; B++ {
		if *closedCircle == true {
			break
		}
		// the edge must exist and the B may not be the way back in the snake
		if graph[A][B] > -1 && (len(snake) == 1 || B != snake[len(snake) - 2]) {
			// check if we can close a circle
			for i, C := range snake {
				if B == C {
					circle := make([]int, len(snake) - i)
					copy(circle, snake[i:])
					*closedCircle = true
					breakCircle(circle)
					break
			...
			if *closedCircle == false {
				// make the snake larger
				newSnake := make([]int, len(snake))
				copy(newSnake, snake)
				newSnake = append(newSnake, B)
				slither(newSnake, closedCircle)
			}
		}
	}
}
...
func removeAllTriangles() {
	...
	for A := 0; A < N; A++ {
		// now we iterate over all possible pairs of neighbors. As the matrix is symmetric, we only need to consider the ones greater than A
		for B := A+1; A != B && B < N; B++ {
			for C := B+1; C != B && C < N; C++ {
				// check if there are edges A-B, A-C and if B-C are connected
				if graph[A][B] > -1 && graph[A][C] > -1 && graph[B][C] > -1 {
					fmt.Println("removing triangle ", A, B, C)
					breakCircle([]int{A,B,C})
				}
			}
		}
	}
...
// given a slice of edges, removes from graph the longest edge in the circle, thus breaking it
func breakCircle(edges []int) {
	biggestEdge := -1
	biggestEdgeA := -1
	biggestEdgeB := -1
	...
	for i, A := range edges {
		...
		if graph[A][B] > biggestEdge {
			biggestEdge = graph[A][B]
			biggestEdgeA = A
			biggestEdgeB = B
		}
	}

	fmt.Println("removing edge ", biggestEdgeA, biggestEdgeB)
	graph[biggestEdgeA][biggestEdgeB] = -1
	graph[biggestEdgeB][biggestEdgeA] = -1
}
